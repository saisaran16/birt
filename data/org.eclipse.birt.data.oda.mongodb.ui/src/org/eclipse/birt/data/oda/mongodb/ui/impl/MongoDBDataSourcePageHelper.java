...
public class MongoDBDataSourcePageHelper
{
	...
	private Button URIElementsRadioBtn, URIRadioBtn, socketAliveCheckbox,useKerberosAuthenticationCheckBox, kerberosConfigurationRadioBtn,kerberosCredentialsRadioBtn;
	private Group URIElementsGroup, kerberosConfigElementGroup;
	private Text serverHostText, serverPortText, databaseNameText,userNameText, passwordText, databaseURIText, gssapiServiceNameText,kerberosConfigurationFileText, gssAPIConfigurationFileText,kerberosPrincipalText, kerberosPasswordText;

	private boolean isPropertyPage, isURITextFieldFoucs, socketAlive,useKerberosAuthentication, isKerberosPasswordFieldFoucs;
	private String dbURI, serverHost, serverPort, dbName, userName, password,gssapiServiceName, kerberosConfigurationFile,gssAPIConfigurationFile, kerberosPrincipal, kerberosPassword;
	...
	private void createKerberosSettingsArea( Composite composite )
	{
		...
		useKerberosAuthenticationCheckBox.setText( Messages.getString( "MongoDBDataSourceWizardPage.checkbox.label.KerberosAuthentication" ) ); //$NON-NLS-1$
		 ...
		 * kerberosSettingComposite3, SWT.RADIO ); GridData radioBtnData = new GridData( ); radioBtnData.verticalAlignment = SWT.TOP;
		 * 
		...
		kerberosConfigElementGroup.setText( Messages.getString( "MongoDBDataSourceWizardPage.RadioButton.label.KerberosConfiguration" ) ); //$NON-NLS-1$
		...
		kerberosConfigurationFileLabel.setText( Messages.getString( "MongoDBDataSourceWizardPage.text.label.KerberosConfigurationFile" ) ); //$NON-NLS-1$
		...
		gssAPIConfigurationFileLabel.setText( Messages.getString( "MongoDBDataSourceWizardPage.text.label.GssAPIConfigurationFile" ) ); //$NON-NLS-1$
	...
	protected void refreshPageControls( )
	{
		...
		if ( useKerberosAuthentication )
		{
			...
			kerberosConfigurationFileText.setText( kerberosConfigurationFile == null ? EMPTY_STRING : kerberosConfigurationFile );
	...
	private boolean validatePageProperties( )
	{
		...
		else if ( serverHostText.getText( ).trim( ).length( ) == 0 || serverPortText.getText( ).trim( ).length( ) == 0 || databaseNameText.getText( ).trim( ).length( ) == 0 )
		{
			isValid = false;
		}
		// Kerberos validation
		...
		if ( !isValid )
		{
			if ( isPropertyPage )
			{
				propertyPage.setMessage( Messages.getString( "MongoDBDataSourceWizardPage.error.MissingConnectionProperty" ), //$NON-NLS-1$
	...
	protected Properties collectCustomProperties( Properties properties )
	{
		...
		// System.out.println( "URIElementsRadioBtn.getSelection( ) :: " +
