...
public class QueryModel
{
	...
	private static final String[] REQUIRED_MAPREDUCE_KEYS = new String[]{MAP_REDUCE_MAP_FUNCTION, MAP_REDUCE_REDUCE_FUNCTION, "out" //$NON-NLS-1$
	};
	...
	{
		if ( cmdOp != CommandOperationType.AGGREGATE && cmdOp != CommandOperationType.MAP_REDUCE && cmdOp != CommandOperationType.RUN_DB_COMMAND )
			return; // nothing to validate
	...
	private static void validateDBCommand( DBObject commandObj )throws OdaException
	{
		...
		if ( commandObj.containsField( EVAL_KEY ) )
		{
			...
			if ( noLockValue != true )
				throw new OdaException( Messages.bind( Messages.queryModel_invalidDbCmdKeyValue,EVAL_KEY,"{" + NOLOCK_KEY + " : true}" ) ); //$NON-NLS-1$ //$NON-NLS-2$
		}
	}
